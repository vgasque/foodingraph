...
From an adjacency matrix, the package can infer the network with confidence-interval (CI) bootstraps of the distribution of mutual information[^1] values or maximal information coefficents (MIC)[^2]for each pairwise association.
...
From the infered adjacency matrix, the package can then display the graph using `ggplot2`[^3], `igraph`[^4] and `ggraph`[^5].
...
As the computations can take some time, a progress bar is built into the fucntion. You cand deactivate it by setting the parameter `show_progress` to FALSE (function `boot_cat_bin`).
...
n_links_before <- (length(which(adjacency_matrix!=0))-ncol(obs_data))/2
n_links_after <- (length(which(inferred_adj_matrix!=0))-ncol(obs_data))/2
print(paste(n_links_before - n_links_after, "edges have been removed"))
```
